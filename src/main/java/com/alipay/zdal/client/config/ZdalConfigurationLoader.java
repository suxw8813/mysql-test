...
public class ZdalConfigurationLoader{
    ...
    private static final Logger log = Logger.getLogger(Constants.CONFIG_LOG_NAME_LOGNAME);
    
    private static final ZdalConfigurationLoader instance = new ZdalConfigurationLoader();
    
    /** pkey=appName value=(key = appDsName,value = zdalconfig ) */
     ...
     * @return
     */
    public synchronized ZdalConfig getZdalConfiguration(String appName, String dbMode, String appDsName, String configPath){
        Map<String, ZdalConfig> configs = appDsConfigs.get(appName);
        if(configs == null || configs.isEmpty()){
            Map<String, ZdalConfig> maps = getZdalConfigurationFromLocal(appName, dbMode, appDsName, configPath);
     ...
     * @return
     */
    private synchronized Map<String, ZdalConfig> getZdalConfigurationFromLocal(String appName, String dbMode, String appDsName, String configPath){
     ...
     * @param idcName
     */
    private synchronized Map<String, ZdalConfig> loadZdalConfigurationContext(String[] fileNames, String appName, String dbMode){
        ...
        try{
            ...
            if(null == ctx.getBean(appName)){
                throw new ZdalConfigException("ERROR ## It must has at least one app bean in " + appName + " Zdal configuration file ");
            }
            ZdalAppBean appBean = (ZdalAppBean)ctx.getBean(appName);
            if(CollectionUtils.isEmpty(appBean.getAppDataSourceList())){
                throw new ZdalConfigException("ERROR ## It must has at least one appDataSource bean in " + appName + " ZdalAppBean");
            }
            if(!appName.equals(appBean.getAppName())){
                throw new ZdalConfigException("ERROR ## the configured appName is " + appBean.getAppName()
                                              + " are not match with requested appName:"
                                              + appName);
            }
            if(!dbMode.equals(appBean.getDbmode())){
                throw new ZdalConfigException("ERROR ## The configured dbMode is " + appBean.getDbmode() + " are not match with requested dbMode: " + dbMode);
            }
            
            for(AppDataSourceBean appDataSourceBean : appBean.getAppDataSourceList()){
                zdalConfigMap.put(appDataSourceBean.getAppDataSourceName(), populateZdalConfig(appBean, appDataSourceBean));
            }
    ...
    protected ZdalConfig populateZdalConfig(ZdalAppBean appBean, AppDataSourceBean appDataSourceBean){
        ...
        for(PhysicalDataSourceBean physicalDataSource : appDataSourceBean.getPhysicalDataSourceSet()){
            config.getDataSourceParameters().put(physicalDataSource.getName(), DataSourceParameter.valueOf(physicalDataSource));
            if(null != physicalDataSource.getLogicDbNameSet() && !physicalDataSource.getLogicDbNameSet().isEmpty()){
